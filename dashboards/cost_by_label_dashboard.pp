...
query "cost_by_label_dashboard_total_cost" {
  sql = <<-EOQ
    ...
    from picked
    where label_value is not null and label_value <> '';
  EOQ
  ...
  sql = <<-EOQ
    with filtered as (
      select date_trunc('month', usage_start_time) as month,
             cost, project_labels, labels, system_labels, project_id
      from gcp_billing_report
      where ('all' in ($1) or project_id in $1)
    ),
    picked as (
      select f.month, f.cost,
             json_extract_string(f.project_labels, '$[' || i || '].value') as label_value
    ...
    from picked
    where label_value is not null and label_value <> ''
    group by month, label_value
    having sum(cost) > 0
    order by month, sum(cost) desc;
  EOQ
...
query "cost_by_label_dashboard_top_10_label_values" {
  sql = <<-EOQ
    ...
    picked as (
      select json_extract_string(f.project_labels, '$[' || i || '].value') as label_value, cost
      from filtered f, generate_series(0, 200) as gs(i)
      where $2 = 'project_labels'
        and f.project_labels is not null
        and i < json_array_length(f.project_labels)
      ...
      select json_extract_string(f.labels, '$[' || i || '].value'), cost
      from filtered f, generate_series(0, 200) as gs(i)
      where $2 = 'labels'
        and f.labels is not null
        and i < json_array_length(f.labels)
      ...
      select json_extract_string(f.system_labels, '$[' || i || '].value'), cost
      from filtered f, generate_series(0, 200) as gs(i)
      where $2 = 'system_labels'
        and f.system_labels is not null
        and i < json_array_length(f.system_labels)
  ...
  sql = <<-EOQ
    with filtered as (
      select project_id, project_name, location.region as region,
             cost, currency, project_labels, labels, system_labels
      from gcp_billing_report
      where ('all' in ($1) or project_id in $1)
    ),
    picked as (
      select project_id, project_name, region, currency, cost,
             json_extract_string(f.project_labels, '$[' || i || '].value') as label_value
      from filtered f, generate_series(0, 200) as gs(i)
      where $2 = 'project_labels'
        and f.project_labels is not null
    ...
    from picked
    where label_value is not null and label_value <> ''
    group by label_value, project_id, project_name, region
    order by sum(cost) desc;
  EOQ
...
query "cost_by_label_dashboard_projects_input" {
  sql = <<-EOQ
    ...
    select 'All' as label, 'all' as value
    union all
    select label, value
    from labeled
    order by label;
